...
/* Snapshot of files as they were immediately before the most
   recent AI edit, used to power the "Revert" button. Cleared
   whenever the user reverts, switches projects, or a fresh AI
   edit overwrites it with a newer snapshot. */

let lastAiChangeSnapshot = null;
...
const revertAiChangeBtn =
    document.getElementById("revertAiChangeBtn");
...
function getFileIcon(filename) {
    ...
    if (filename.endsWith(".html")) {
        return `<svg class="icon" viewBox="0 0 24 24"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>`;
    }

    if (filename.endsWith(".css")) {
        return `<svg class="icon" viewBox="0 0 24 24"><path d="M12 2s7 8.5 7 13a7 7 0 0 1-14 0c0-4.5 7-13 7-13Z"/></svg>`;
    }

    if (filename.endsWith(".js")) {
        return `<svg class="icon" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>`;
    }

    if (filename.endsWith(".json")) {
        return `<svg class="icon" viewBox="0 0 24 24"><path d="M8 3a2 2 0 0 0-2 2v4a2 2 0 0 1-2 2 2 2 0 0 1 2 2v4a2 2 0 0 0 2 2"/><path d="M16 3a2 2 0 0 1 2 2v4a2 2 0 0 0 2 2 2 2 0 0 0-2 2v4a2 2 0 0 1-2 2"/></svg>`;
    }

    if (filename.endsWith(".md")) {
        return `<svg class="icon" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="12" y2="17"/></svg>`;
    }

    return `<svg class="icon" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`;
...
async function switchProject(projectId) {
    ...
    /* Switching projects invalidates any pending revert --
       the snapshot belonged to the previous project's files. */

    lastAiChangeSnapshot =
        null;
...
function updateRevertButtonState() {
    ...
    if (!revertAiChangeBtn) {
        return;
    }

    const hasSnapshot =
        !!lastAiChangeSnapshot &&
        Object.keys(lastAiChangeSnapshot).length > 0;

    revertAiChangeBtn.disabled =
        !hasSnapshot;

    revertAiChangeBtn.style.display =
        hasSnapshot
            ? "flex"
            : "none";
...
function revertLastAiChange() {
    ...
    Object.keys(snapshot).forEach(
        filename => {
            ...
            if (previousFile === null) {
                ...
                /* This file didn't exist before the AI
                   edit -- the AI created it, so reverting
                   removes it entirely. Never remove the
                   last remaining file. */
    ...
    /* This snapshot has been consumed -- only one level of
       undo is supported, so clear it and disable the button
       until the next AI edit creates a new snapshot. */
...
if (revertAiChangeBtn) {
    ...
    revertAiChangeBtn.addEventListener(
        "click",
        revertLastAiChange
    );
...
async function processAIRequest(request) {
    ...
    try {
        ...
        if (
            ...
            /* Snapshot every file about to be touched,
               BEFORE any of them are overwritten, so the
               user can revert this whole AI edit as one
               step. A snapshot value of null means the file
               didn't exist before this change (i.e. the AI
               created it), so reverting should remove it. */
            ...
            lastAiChangeSnapshot =
                Object.keys(preChangeSnapshot).length > 0
                    ? preChangeSnapshot
                    : lastAiChangeSnapshot;
...
/* =========================================================
   MOBILE DRAWERS (sidebar + AI panel)
========================================================= */
...
function closeMobileDrawers() {
    ...
    if (aiPanelElement) {
        ...
        aiPanelElement.classList.remove(
            "mobile-open"
        );
...
function openMobileDrawer(panelEl) {
...
if (chatToggleBtn) {
    ...
    chatToggleBtn.addEventListener(
        ...
        () => {
            ...
            const isOpen =
                aiPanelElement &&
                aiPanelElement.classList.contains(
                    "mobile-open"
                );
...
async function renderProjectsList() {
    ...
    projectsList.innerHTML = `
        <div class="projects-loading">
            <span class="skeleton-bar" style="width: 60%; margin: 0 auto 10px;"></span>
            <span class="skeleton-bar" style="width: 40%; margin: 0 auto;"></span>
        </div>
    `;
